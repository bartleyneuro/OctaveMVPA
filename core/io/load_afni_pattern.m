...
if isstr(filenames)
	if ~is_absolute_filename(filenames)
		filenames = make_absolute_filename(filenames);
	end
else
	for i=1:length(filenames)
		if ~is_absolute_filename(filenames{i})
			filenames{i} = make_absolute_filename(filenames{i});
		end
	end
end
